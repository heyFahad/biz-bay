...
class Login extends Component {
    ...
    focusBlurredHandler = (event, formElementId) => {
        const formElement = this.state.loginForm[formElementId]
        const updatedFormElement = updateObject(formElement, {
            isValid: checkInputFieldValidity(event.target.value, this.state.loginForm[formElementId].validationRules),
            isFocused: false
        });
        const updatedLoginForm = updateObject(this.state.loginForm, {
            [formElementId]: updatedFormElement
    ...
    loginRequestHandler = (event) => {
        ...
        firebase.doSignInWithEmailAndPassword(formData)
            .then(() => {
                const clearedEmail = updateObject(this.state.loginForm.email, { value: '' });
                const clearedPassword = updateObject(this.state.loginForm.password, { value: '' });
                const updatedLoginForm = updateObject(this.state.loginForm, {
                    email: clearedEmail,
                    ...
                    inputElementsArray.map(inputElement => {
                        return (
                            <Input
                                ...
                                blurred={(event) => this.focusBlurredHandler(event, inputElement.id)} />
                        );
                    })
                }
