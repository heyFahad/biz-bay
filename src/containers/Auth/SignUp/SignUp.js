...
class SignUp extends Component {
    ...
    signUpRequestHandler = (formData) => {
        ...
        firebase.doSignInWithPhoneNumber({ formData: formData, applicationVerifier: this.recaptchaVerifier })
    ...
    confirmPhoneAuthVerificationCode = ({ verificationCode }) => {
        ...
        this.confirmationResult.confirm(verificationCode)
            .then(userCredential => {
                ...
                const emailAuthCredential = firebase.doCreateEmailPasswordAuthCredential(this.signUpFormData);
                firebase.doLinkWithCredential({ credential: emailAuthCredential })
                    .then(userCredential => {
                        console.log({ userCredential });
                        firebase.currentUser = userCredential.user;
                        firebase.doUserProfileUpdate(this.signUpFormData)
                            .then(() => {
    ...
    render() {
        return (
            <Modal
                ...
                backdropClicked={this.props.hide}>
